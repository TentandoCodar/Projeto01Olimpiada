const data = [
  {
    image: 'data:image/png;base64,',
    description: 'Curso intensivo sobre as novas funcionalidades do ES6',
    extendData: 'Curso focado em arrowFunctions, Promisses e Desestruturação',
  },
  {
    image: 'https://s3-ap-south-1.amazonaws.com/av-blog-media/wp-content/uploads/2019/08/python_tricks.jpg',
    description: 'Curso intensivo sobre data Science com python',
    extendData: 'Curso focado em MATPLOITLIB e na biblioteca pandas',
  },
  {
    image: 'https://miro.medium.com/max/918/1*YU6BvZKvxivoEnvqxeG5rw.png',
    description: 'Curso intensivo de logica de programação',
    extendData: 'Curso focado no basico de programação',
  },
  {
    image: 'https://inoveimpressao.com.br/wp-content/uploads/2018/05/design-grafico.jpg',
    description: 'Curso intensivo sobre design grafico e seus conceitos',
    extendData: 'Curso focado em arrowFunctions, Promisses e Desestruturação',
  },
  {
    image: 'https://rockcontent.com/wp-content/uploads/2016/06/como-usar-excel-1280x720.png',
    description: 'Curso de excel 2019 com pacote gratis e suporte individual',
    extendData: 'Curso sobre produtividade com excel, desde o basico ao avançado',
  },
  {
    image: 'https://miro.medium.com/max/3000/1*30aoNxlSnaYrLhBT0O1lzw.png',
    description: 'Curso de backend com GOLANG e seus conceitos',
    extendData: 'Uma linguagem rapida, performatica e simples',
  },
  {
    image: 'https://miro.medium.com/max/1200/1*eLqeIits5crU3G5b9LMEyg.png',
    description: 'Curso de Clojure, do basico ao avançado, o dialeto mais conhecido de lisp',
    extendData: 'Clojure é um dialeto da linguagem de programação Lisp criado por Rich Hickey. ',
  },
  {
    image: 'https://cdn.vox-cdn.com/thumbor/_AobZZDt_RVStktVR7mUZpBkovc=/0x0:640x427/1200x800/filters:focal(0x0:640x427)/cdn.vox-cdn.com/assets/1087137/java_logo_640.jpg',
    description: 'Curso de Java, a linguagem mais utilizada do mundo',
    extendData: 'Java, a linguagem utilizada por grandes como amazon, google e twitter. ',
  },
  {
    image: 'https://dkrn4sk0rn31v.cloudfront.net/2018/01/22145352/ONDE-COMECAR-C-SHARP.png',
    description: 'Curso de C#, do basico ao avançado, a linguagem da Microsoft',
    extendData: 'Aprenda a linguagem c# fazendo jogos na unity',
  },
]

export default data;